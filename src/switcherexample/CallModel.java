 ...
 * @author Sam Richards
 */
public class CallModel extends Guassian implements CommonEquations
{
    ...
    {
        callPrice = this.stock*exp(-1*this.dividend*this.time)*Guassian.cdf(d1) - this.strike*exp(-1*this.interest * this.time)*Guassian.cdf(d2);
        value = String.format("%.8f", callPrice);
        return (value);
